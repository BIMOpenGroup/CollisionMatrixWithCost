import { llmRequest, ChatMessage, chatCompletionOpenAICompatible } from './llm/client'

export { chatCompletionOpenAICompatible }
...
Ответь строго JSON‑массивом [{index, score}] с индексами из списка кандидатов; score в диапазоне 0..1.`
  ...
  return llmRequest('rerank', [{ role: 'system', content: system }, { role: 'user', content: user }], parseRankJson, 0.2)
}
...
${candidates
  ...
  return llmRequest('decide:cell', [{ role: 'system', content: system }, { role: 'user', content: user }], parseDecisionJson, 0.1)
}
...
${candidates
  ...
  return llmRequest('decide:element', [{ role: 'system', content: system }, { role: 'user', content: user }], parseElementDecisionJson, 0.1)
}
...
Колонка: ${colItems.map((i) => `${i.name}${i.unit ? ` (${i.unit})` : ''}${typeof i.price === 'number' ? `; price=${i.price}` : ''}`).join('; ')}`
  ...
  return llmRequest('collision', [{ role: 'system', content: system }, { role: 'user', content: user }], (content) => {
    try {
      const parsed = JSON.parse(content)
      ...
      const scenarios_json = sc ? JSON.stringify(sc) : undefined
...
Колонка: ${colItems.map((i) => `${i.name}${i.unit ? ` (${i.unit})` : ''}${typeof i.price === 'number' ? `; price=${i.price}` : ''}`).join('; ')}`
  ...
  return llmRequest('risk', [{ role: 'system', content: system }, { role: 'user', content: user }], (content) => {
    try {
      const parsed = JSON.parse(content)
      const hazard = Number(parsed?.hazard)
      const importance = Number(parsed?.importance)
      const difficulty = Number(parsed?.difficulty)
      const r = parsed?.rationale
      const rationale_json = r ? JSON.stringify(r) : undefined
      return {
